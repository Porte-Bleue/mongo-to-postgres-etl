...
if __name__ == "__main__":
    ...
    with Flow("ETL From MongoDB To Postgres") as flow:
        # Load credentials stored as Secrets on Prefect Cloud
        mongodb_secret = PrefectSecret("PREFECT__MONGO_DB")
        postgre_secret = PrefectSecret("PREFECT__POSTGRES_DB")
        for t in tables_to_extract:
            print(f"extracting collection: {t}")
            t1 = extract(mongodb_secret, collection_name=t)
            print(f"loading table: {t}")
            t2 = load(postgre_secret, table_name=t, upstream_tasks=[t1])
